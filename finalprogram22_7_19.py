...
ascii.write(r,'gaia.csv',exclude_names=['designation'],delimiter=',')    #saving gaia data in gaia.csv file 
...
ww=len(w)   #just to know how many gaia stars in region
values={'pm':0.00005}  #dealing with stars with no proper motion
gaia=gaia.dropna(subset=['ra','dec','pmra','pmdec'])  # accessing stars only having four parameters
...
for i in range(len(mystixra)):	#mystixdata
	...
	for j in range(len(gaiara)):	#gaiadata
		if mystixdec[i]-r<gaiadec[j] and gaiadec[j]<mystixdec[i]+r :
			...
			ram=mystixra[i]
			decm=mystixdec[i]
			d=2*math.asin((math.sin((decm-decg)*0.5))**2+((math.sin((ram-rag)*0.5))**2)*(math.cos(decg))*(math.cos(decm))) #haversine formula
			if d < r:
...
#finding possible duplicates in MYStIX and GAIA
dpm=[]
dpg=[]
best1=set(best)
best2=list(best1)
...
print len(cluster1),ww,len(gaiara),len(dpg)
